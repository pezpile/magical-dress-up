// Base body — always equipped
export const BASE_ASSETS = { 0: { src: '/assets/base.png' } };

// Eye sets — { sclera?, iris } paths
export const EYES_ASSETS = {
  0: { sclera: '/assets/eyes/sclera1.png', iris: '/assets/eyes/eye1.png' },
  1: { iris: '/assets/eyes/eye2.png' },
  2: { iris: '/assets/eyes/eye3.png' },
  3: { iris: '/assets/eyes/eye4.png' },
  4: { iris: '/assets/eyes/eye5.png' },
  5: { iris: '/assets/eyes/eye6.png' },
};

// All other assets use { src }
export const EYEBROWS_ASSETS = {
  0: { src: '/assets/eyes/eyebrow1.png' },
  1: { src: '/assets/eyes/eyebrow2.png' },
  2: { src: '/assets/eyes/eyebrow3.png' },
  3: { src: '/assets/eyes/eyebrow4.png' },
  4: { src: '/assets/eyes/eyebrow5.png' },
  5: { src: '/assets/eyes/eyebrow6.png' },
};
export const MOUTH_ASSETS = {
  0: { src: '/assets/mouth/mouth1.png' },
  1: { src: '/assets/mouth/mouth2.png' },
  2: { src: '/assets/mouth/mouth3.png' },
  3: { src: '/assets/mouth/mouth4.png' },
  4: { src: '/assets/mouth/mouth5.png' },
  5: { src: '/assets/mouth/mouth6.png' },
  6: { src: '/assets/mouth/mouth7.png' },
};
export const NOSE_ASSETS = {
  0: { src: '/assets/nose/nose.png' },
  1: { src: '/assets/nose/nose2.png' },
  2: { src: '/assets/nose/nose3.png' },
  3: { src: '/assets/nose/nose4.png' },
};

export const BANGS_ASSETS = {
  0: { src: '/assets/hairbang/bang1.png' },
  1: { src: '/assets/hairbang/bang2.png' },
  2: { src: '/assets/hairbang/bang3.png' },
  3: { src: '/assets/hairbang/bang4.png' },
  4: { src: '/assets/hairbang/bang5.png' },
  5: { src: '/assets/hairbang/bang6.png' },
  6: { src: '/assets/hairbang/bang7.png' },
};
export const HAIR_BACK_ASSETS = {
  0: { src: '/assets/hairback/hairback1.png' },
  1: { src: '/assets/hairback/hairback2.png' },
  2: { src: '/assets/hairback/hairback3.png' },
  3: { src: '/assets/hairback/hairback4.png' },
  4: { src: '/assets/hairback/hairback5.png' },
};
export const BUNS_ASSETS = {
  0: { src: '/assets/hairextras/buns1.png' },
  1: { src: '/assets/hairextras/buns2.png' },
  2: { src: '/assets/hairextras/buns3.png' },
  3: { src: '/assets/hairextras/buns4.png' },
};

export const SHIRT_ASSETS = {
  0: { src: '/assets/shirt/shirt1.png' },
  1: { src: '/assets/shirt/shirt2.png' },
  2: { src: '/assets/shirt/shirt3.png' },
  3: { src: '/assets/shirt/shirt4.png' },
  4: { src: '/assets/shirt/shirt5.png' },
  5: { src: '/assets/shirt/shirt6.png' },
};
export const PANT_ASSETS = {
  0: { src: '/assets/pants/pant1.png' },
  1: { src: '/assets/pants/pant2.png' },
  2: { src: '/assets/pants/pant3.png' },
  3: { src: '/assets/pants/pant4.png' },
  4: { src: '/assets/pants/pant5.png' },
};
export const SOCK_ASSETS = {
  0: { src: '/assets/socks/sock1.png' },
  1: { src: '/assets/socks/sock2.png' },
  2: { src: '/assets/socks/sock3.png' },
  3: { src: '/assets/socks/sock4.png' },
};
export const SHOE_ASSETS = {
  0: { src: '/assets/shoes/shoe1.png' },
  1: { src: '/assets/shoes/shoe2.png' },
  2: { src: '/assets/shoes/shoe3.png' },
  3: { src: '/assets/shoes/shoe4.png' },
  4: { src: '/assets/shoes/shoe5.png' },
  5: { src: '/assets/shoes/shoe6.png' },
};

export const NECKLACE_ASSETS = {
  0: { src: '/assets/accessories/necklace1.png' },
  1: { src: '/assets/accessories/necklace2.png' },
};
export const BRACELET_ASSETS = {
  0: { src: '/assets/accessories/bracelet1.png' },
  1: { src: '/assets/accessories/bracelet2.png' },
  2: { src: '/assets/accessories/bracelet3.png' },
  3: { src: '/assets/accessories/armcuff1.png'  },
};
export const EARRING_ASSETS = {
  0: { src: '/assets/accessories/earring1.png' },
  1: { src: '/assets/accessories/earring2.png' },
  2: { src: '/assets/accessories/earring3.png' },
  3: { src: '/assets/accessories/earring4.png' },
  4: { src: '/assets/accessories/earring5.png' },
  5: { src: '/assets/accessories/earring6.png' },
};
export const BELT_ASSETS = {
  0: { src: '/assets/accessories/belt1.png' },
  1: { src: '/assets/accessories/belt2.png' },
  2: { src: '/assets/accessories/belt3.png' },
  3: { src: '/assets/accessories/belt4.png' },
};
export const RING_ASSETS      = { 0: { src: '/assets/accessories/ring1.png'       } };
export const ARMWARMER_ASSETS = {
  0: { src: '/assets/accessories/armwarmer1.png' },
  1: { src: '/assets/accessories/armwarmer2.png' },
};
export const BELLY_ASSETS     = { 0: { src: '/assets/accessories/bellystud1.png'  } };

export const HAT_ASSETS      = { 0: { src: '/assets/headwear/hat1.png'       } };
export const HAIRCLIP_ASSETS = {
  0: { src: '/assets/headwear/hairclip1.png'  },
  1: { src: '/assets/headwear/hairflower1.png' },
};
export const HAIRTIE_ASSETS = {
  0: { src: '/assets/headwear/hairclip2.png'  },
  1: { src: '/assets/headwear/bow1.png'        },
  2: { src: '/assets/headwear/scrunchie1.png'  },
  3: { src: '/assets/headwear/hairpuffs1.png'  },
};

export const MARKS_ASSETS = {
  0: { src: '/assets/face/freckles1.png' },
  1: { src: '/assets/face/moles1.png'    },
};

export const DRESS_ASSETS = {
  0: { src: '/assets/dress/dress1.png' },
  1: { src: '/assets/dress/dress2.png' },
};
